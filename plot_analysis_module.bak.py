...
def plot_four_panel(merge_file, title=''):
    ...
    for i in range(4):
        plt.sca(axs[i])
        plot_hex1to1(merge_file['analysis/flat/' + var[i] + '_truth'][:],
                        merge_file['analysis/flat/'+var[i]][:],
                        'Truth '+var[i]+unit[i],
...
def plot_four_panel_kde(merge_file, comp_file, centered=True, title=''):
    '''
    ...
    axs = axs.ravel()
    for i in range(4):
        merge_var = merge_file['analysis/flat'][var[i]][np.logical_not(np.isnan(merge_file['analysis/flat'][var[i]][:]))]
        ...
        comp_var = comp_file['analysis/flat'][var[i]][np.logical_not(np.isnan(comp_file['analysis/flat'][var[i]][:]))]
...
def plot_four_panel_spatial(merge_file, lon_grid, lat_grid, site,  title=''):
    '''
    ...
    axs = axs.ravel()
    for i in range(4):
        merge_var = np.abs(merge_file['analysis/flat'][var[i]][:])
        ...
        axs[i].set_title(r'%s, $\mu$=%.2f, $\sigma$=%.2f' % (var[i], var_med, var_std) )
...
def plot_medians(merge_file, comp_file, xvars, yvars, mergelbl='', complbl='', plot_directory='./', apply_ppf=False):
    ...
    for xvar in xvars:
        if apply_ppf:
            merge_xvar = merge_file['analysis/flat'][xvar][ind_ppf_merge]
            comp_xvar = comp_file['analysis/flat'][xvar][ind_ppf_comp]
        else:
            merge_xvar = merge_file['analysis/flat'][xvar][:]
            comp_xvar = comp_file['analysis/flat'][xvar][:]
        ...
        for yvar in yvars:
            if apply_ppf:
                merge_yvar = merge_file['analysis/flat'][yvar][ind_ppf_merge]
                comp_yvar = comp_file['analysis/flat'][yvar][ind_ppf_comp]
            else:
                merge_yvar = merge_file['analysis/flat'][yvar][:]
                comp_yvar = comp_file['analysis/flat'][yvar][:]
...
def plot_medians_ppf(merge_file, xvars, yvars, mergelbl='', plot_directory='./', filteredlbl='ppf'):
    ...
    '''
    for xvar in xvars:
        selected_xvar = merge_file['analysis/flat'][xvar][:]
        ...
        for yvar in yvars:
            selected_yvar = merge_file['analysis/flat'][yvar][:]
...
def plot_se_medians(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./'):
    '''
    ...
    for xvar in xvars:
        xvar_std = np.nanstd(selected_xvar)
        xvar_med = np.nanmedian(merge_file['analysis/flat'][xvar][:])
        xvar_plotmax = xvar_med+xvar_std*5
        xvar_plotmin = np.nanmin(merge_file['analysis/flat'][xvar][:])
        cv_bins = np.linspace(xvar_plotmin, xvar_plotmax, 11)
        binned_out = np.digitize(merge_file['analysis/flat'][xvar][:], bins=cv_bins)
        for yvar in yvars:
            N_merge = np.sum(np.logical_not(np.isnan(merge_file['analysis/flat'][yvar][:]))*1)
            N_comp = np.sum(np.logical_not(np.isnan(comp_file['analysis/flat'][yvar][:]))*1)
            compmeans = np.full(11, np.nan)
            mergemeans = np.full(11, np.nan)
            for i in np.arange(1,11):
                binind_out = (binned_out == i)
                if np.sum(binind_out*1) > 10:
                    compmeans[i-1] = np.nanmedian(comp_file['analysis/flat'][yvar][binind_out])
                    mergemeans[i-1] = np.nanmedian(merge_file['analysis/flat'][yvar][binind_out])
            fig, ax1 = plt.subplots(1,1, figsize=(6,5))
            ax1.scatter(x=merge_file['analysis/flat'][xvar][:],y=merge_file['analysis/flat'][yvar][:],
                        alpha=0.3,s=10, color='royalblue')
            ax1.scatter(x=comp_file['analysis/flat'][xvar][:],y=comp_file['analysis/flat'][yvar][:],
                        alpha=0.3,s=10, color='lightcoral')
            ax1.plot(cv_bins, compmeans, '--sr', alpha=0.8, label=complbl)
            ax1.plot(cv_bins, mergemeans, '--ob', alpha=0.8, label=mergelbl)
            ax1.set_ylabel(yvar)
            ax1.set_xlabel(xvar)
            ax1.set_xticks(cv_bins)
            ax1.set_xticklabels(np.round(cv_bins,4), rotation=90)
            ax1.set_xlim([xvar_plotmin, np.nanmax(mergemeans)])
            ax1.legend()
            ind = np.logical_not(np.isnan(comp_file['analysis/flat'][yvar][:]))
            rho_comp = np.corrcoef(comp_file['analysis/flat'][xvar][ind],comp_file['analysis/flat'][yvar][ind])[0,1]
            ind = np.logical_not(np.isnan(merge_file['analysis/flat'][yvar][:]))
            rho_merge = np.corrcoef(merge_file['analysis/flat'][xvar][ind],merge_file['analysis/flat'][yvar][ind])[0,1]
            plt.title(r'N$_{%s}$ = %d, N$_{%s}$ = %d, $\rho_{%s}$ = %.2f, $\rho_{%s}$ = %.2f'\
                      %(complbl, N_comp, mergelbl, N_merge, complbl, rho_comp, mergelbl, rho_merge))
            plt.savefig(plot_directory + '/shape_error_medians-' + xvar + '_vs_' + yvar)
            plt.close()


def plot_se_scatter(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./'):
    '''
    ...
    xvars = ['ils_shape_error_o2','ils_shape_error_weak_co2','ils_shape_error_strong_co2', 'ils_shape_error_ch4'] 
    for xvar in xvars:
        xvar_std = np.nanstd(merge_file['analysis/flat'][xvar][:])
        xvar_med = np.nanmedian(merge_file['analysis/flat'][xvar][:])
        xvar_plotmax = xvar_med+xvar_std*5
        xvar_plotmin = np.nanmin(merge_file['analysis/flat'][xvar][:])
        cv_bins = np.linspace(xvar_plotmin, xvar_plotmax, 11)
        ...
        for yvar in yvars:
            ...
            for i in np.arange(1,11):
                binind_out = (binned_out == i)
                if np.sum(binind_out*1) >= 10:
                    compstd[i-1] = np.nanstd(comp_file['analysis/flat'][yvar][binind_out])
                    mergestd[i-1] = np.nanstd(merge_file['analysis/flat'][yvar][binind_out])
            fig, ax1 = plt.subplots(1,1, figsize=(6,5))
            ax1.plot(cv_bins, compstd, '--sr', alpha=0.8, label=complbl)
            ax1.plot(cv_bins, mergestd, '--ob', alpha=0.8, label=mergelbl)
            ax1.set_ylabel(yvar + ' scatter')
            ax1.set_xlabel(xvar + ', (w/o SH)')
            ax1.set_xticks(cv_bins)
            ax1.set_xticklabels(np.round(cv_bins,4), rotation=90)
            ax1.legend()
            ind = np.logical_not(np.isnan(compstd))
            ...
            plt.title(r'N$_{%s}$ = %d, N$_{%s}$ = %d, $\rho_{%s}$ = %.2f, $\rho_{%s}$ = %.2f'\
                      %(complbl, N_comp, mergelbl, N_merge, complbl, rho_comp, mergelbl, rho_merge))
            plt.savefig(plot_directory + '/shape_error_scatter-' + xvar + '_vs_' + yvar)
            plt.close()

def plot_cv_se_medians(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./'):
    '''
    ...
    for xvar in xvars:
        ...
        for yvar in yvars:
            ...
            ax1.set_xlim([xvar_plotmin, np.nanmax(mergemeans)])
            ax1.legend()
            rho_comp = np.corrcoef(comp_file['analysis/flat'][xvar][:],comp_file['analysis/flat'][yvar][:])[0,1]
            rho_merge = np.corrcoef(merge_file['analysis/flat'][xvar][:],merge_file['analysis/flat'][yvar][:])[0,1]
            plt.title(r'N$_{%s}$ = %d, N$_{%s}$ = %d, $\rho_{%s}$ = %.2f, $\rho_{%s}$ = %.2f'\
                      %(complbl, N_comp, mergelbl, N_merge, complbl, rho_comp, mergelbl, rho_merge))
            plt.savefig(plot_directory + '/shape_error_medians-' + xvar + '_vs_' + yvar)
            plt.close()
...
def plot_cv_se_scatter(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./'):
    '''
...
def plot_hist_2panel(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./'):
    for key in merge_file['analysis/flat'].keys():
        merge_var = merge_file['analysis/flat'][key][np.logical_not(np.isnan(merge_file['analysis/flat'][key][:]))]
        N_merge = np.size(merge_var)
        mu_merge = np.nanmedian(merge_var)
        sd_merge = np.nanstd(merge_var)
        fig =  plt.figure(figsize=(12,5))
        ax1 = fig.add_subplot(1,2,1)
        ax1.hist(merge_var, edgecolor='white', bins=21)
        ax1.set_title(r'%s, N = %d, $\mu$ = %.3f, $\sigma$ = %.3f'%(mergelbl, N_merge, mu_merge, sd_merge))
        comp_var = comp_file['analysis/flat'][key][np.logical_not(np.isnan(comp_file['analysis/flat'][key][:]))]
        N_comp = np.size(comp_var)
        mu_comp = np.nanmedian(comp_var)
        sd_comp = np.nanstd(comp_var)
        ax2 = fig.add_subplot(1,2,2)
        ax2.hist(comp_var, edgecolor='white', bins=21)
        ax2.set_title(r'%s, N = %d, $\mu$ = %.3f, $\sigma$ = %.3f'%(complbl, N_comp, mu_comp, sd_comp))
        fig.suptitle(key)
        fig.tight_layout()
        plt.savefig(plot_directory + '/hist-' + key)
        plt.close()
...
def plot_spatial(merge_file, comp_file, lon_grid, lat_grid, site, mergelbl='', complbl='', plot_directory='./'):
    for key in merge_file['analysis/flat'].keys():
        fig =  plt.figure(figsize=(12,5))
        ax1 = fig.add_subplot(1,2,1)
        merge_var = merge_file['analysis/flat'][key][:]
        comp_var = comp_file['analysis/flat'][key][:]
...
def plot_kde(merge_file, comp_file, mergelbl='', complbl='', plot_directory='./', centered=True):
    for key in merge_file['analysis/flat'].keys():
        merge_key = merge_file['analysis/flat'][key][np.logical_not(np.isnan(merge_file['analysis/flat'][key][:]))]
        ...
        if not centered:
            ...
            z_merge = (merge_key-mu_merge)
        fig =  plt.figure(figsize=(8,5))
        ax1 = fig.add_subplot(1,1,1)
        sns.kdeplot(z_merge, shade=True, ax=ax1, label=r'w/ sh, N = %d, $\mu$ = %.4e, $\sigma$ = %.4e'%( N_merge, mu_merge, sd_merge))
        comp_key = comp_file['analysis/flat'][key][np.logical_not(np.isnan(comp_file['analysis/flat'][key][:]))]
        ...
        if not centered:
            ...
            ax1.set_title(key + ' (centered)')
        plt.savefig(plot_directory + '/hist-anomaly_' + key)
        plt.close()

def plot_error_correlation(merge_file, keys, plot_directory='./'):
    ind = np.logical_not(np.isnan(merge_file['analysis/flat']['xco2'][:]))
    flat_dat = np.zeros([sum(ind*1), len(keys)])
    for i in range(len(keys)):
        key = keys[i]
        flat_dat[:,i] = merge_file['analysis/flat'][key][ind]
    # pairs data with labels for easy sorting
    df = pd.DataFrame(flat_dat, columns=keys)
    corrmat = df.corr()
...
def post_processing_filter0(merge_file):
    ...
    '''
    chi0 = merge_file['analysis/flat/reduced_chi_squared_o2_fph'][:]
    chi1 = merge_file['analysis/flat/reduced_chi_squared_weak_co2_fph'][:]
    chi2 = merge_file['analysis/flat/reduced_chi_squared_strong_co2_fph'][:]
    chi3 = merge_file['analysis/flat/reduced_chi_squared_ch4_fph'][:]
    chisum = chi0 + chi1 + chi2 + chi3
    dofco2 = merge_file['analysis/flat/dof_co2_profile'][:]
